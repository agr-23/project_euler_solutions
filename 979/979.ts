...
const SUBS_TABLE: number[][] = [
    [0, 0, 1],
    [0, 1],
];

function buildLayers(maxLayer: number): [number[][], number[][], number[][]] {
    const typesList: number[][] = Array.from({length: maxLayer + 1}, () => []);
    const parent1List: number[][] = Array.from({length: maxLayer + 1}, () => []);
    const parent2List: number[][] = Array.from({length: maxLayer + 1}, () => []);
...
function buildBallAdjacency(maxLayer: number): [number[][], number, number[]] {
    ...
    const offsets: number[] = new Array(maxLayer + 1).fill(0);
    let total = 0;
    for (let k = 0; k <= maxLayer; k++) {
        offsets[k] = total;
        total += sizes[k];
...
function computeF(n: number): bigint {
    if (n < 0) return 0n;
    if (n === 0) return 1n;
    const maxLayer = Math.floor(n / 2);
    const [adj, origin] = buildBallAdjacency(maxLayer);
    const nodeCount = adj.length;
    let dp: bigint[] = new Array(nodeCount).fill(0n);
    dp[origin] = 1n;

    for (let step = 0; step < n; step++) {
        const ndp: bigint[] = new Array(nodeCount).fill(0n);
