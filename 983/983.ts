...
function encodePoint(x: number, y: number): number {
    return ((x + OFF) * (1 << SHIFT)) + (y + OFF);
}

function latticePointsOnCircle(m: number): Point[] {
...
function oppositePairs(points: Point[]): [Point, Point][] {
...
function antipodalPairCount(m: number): number {
    let x = m;
...
function displacementSet(points: Point[]): Set<number> {
...
function passesFourVectorPrune(selected: Point[], candidate: Point, deltas: Set<number>): boolean {
...
function evenMasks(k: number): number[] {
    const masks: number[] = [];
...
function centersFromVectors(vectors: Point[], masks: number[]): Point[] {
    const centers: Point[] = [];
...
function quickHarmonyCountEqualsN(centers: Point[], circlePoints: Point[], n: number): boolean {
...
function strictPerfectCheck(centers: Point[], circlePoints: Point[], n: number): boolean {
    ...
    const pointToCenters = new Map<number, number[]>();
    ...
    function union(a: number, b: number): void {
        ...
        if (ra === rb) return;
        if (sz[ra] < sz[rb]) { const tmp = ra; ra = rb; rb = tmp; }
        parent[rb] = ra;
        sz[ra] += sz[rb];
    }
...
function hasUnitCoordinate(points: Point[]): boolean {
...
function hasValidOrientedVectors(pairs: [Point, Point][], circlePoints: Point[], k: number, masks: number[], n: number): boolean {
    ...
    function dfs(start: number): boolean {
        ...
        for (let pairIndex = start; pairIndex <= pairs.length - needed; pairIndex++) {
            const choices: Point[] = selected.length === 0 ? [pairs[pairIndex][0]] : [...pairs[pairIndex]];
...
function findMinRadiusSqForParityFamily(k: number, mLimit: number, filtered: boolean): number {
