...
function buildMulTableQ8(): number[][] {
    const baseArr: number[][] = Array.from({length: 4}, () => new Array(4).fill(0));
    const sgnArr: number[][] = Array.from({length: 4}, () => new Array(4).fill(1));
    for (let a = 0; a < 4; a++) {
        for (let b = 0; b < 4; b++) {
            if (a === 0) {
                baseArr[a][b] = b;
    ...
    const mulTable: number[][] = Array.from({length: 8}, () => new Array(8).fill(0));
    for (let A = 0; A < 8; A++) {
        const sa = A < 4 ? 1 : -1;
        const a = A & 3;
        for (let B = 0; B < 8; B++) {
...
const MUL_TABLE: number[][] = buildMulTableQ8();
const GEN_ELEMS: number[] = [1, 2, 7];
const R_TABLE: number[] = new Array(8 * 3).fill(0);
for (let v = 0; v < 8; v++) {
    for (let b = 0; b < 3; b++) {
        R_TABLE[v * 3 + b] = MUL_TABLE[v][GEN_ELEMS[b]];
    }
}
const INV_TABLE: number[] = new Array(8).fill(0);
for (let e = 0; e < 8; e++) {
    for (let f = 0; f < 8; f++) {
...
function computeF(nVal: number): bigint {
    const MOD = 888888883;
    const MULT = 8888;
    let aVal = 88888888;
    const cnts: number[] = new Array(8).fill(0);
    const R = R_TABLE;
