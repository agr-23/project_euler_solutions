...
const MOD: bigint = 1234567891n;

function trim(poly: bigint[]): bigint[] {
...
function addTo(dst: bigint[], src: bigint[], modulus: bigint | null): void {
...
function mulOneMinusQ(poly: bigint[], modulus: bigint | null): bigint[] {
    const out: bigint[] = new Array(poly.length + 1).fill(0n);
    for (let i = 0; i < poly.length; i++) {
        out[i] += poly[i];
        out[i + 1] -= poly[i];
    }
    const result: bigint[] = modulus !== null ? out.map(v => v % modulus) : out;
    return trim(result);
}

function mulPoly(a: bigint[], b: bigint[], maxDegree: number, modulus: bigint | null): bigint[] {
    if (!a.length || !b.length) return [0n];
    const outLen = Math.min(a.length + b.length - 2, maxDegree) + 1;
    const out: bigint[] = new Array(outLen).fill(0n);
...
function blockCount(length: bigint, cost: bigint): bigint {
...
function blockNumerator(length: number, modulus: bigint | null): bigint[] {
    const coeffs: bigint[] = new Array(length + 1).fill(0n);
...
function numeratorForAllValidVectors(n: number, modulus: bigint | null): bigint[] {
    const blockNum: (bigint[] | null)[] = new Array(n + 1).fill(null);
    for (let length = 2; length <= n; length++) {
        blockNum[length] = blockNumerator(length, modulus);
    }

    const total: bigint[][] = Array.from({ length: n + 1 }, () => []);
    const zeroEnd: bigint[][] = Array.from({ length: n + 1 }, () => []);
    total[0] = [1n];
    zeroEnd[0] = [1n];
...
function countTuples(n: number, k: number, modulus: bigint | null): bigint {
    const nB = BigInt(n);
    const kB = BigInt(k);
    const maxCost = kB / 2n;
