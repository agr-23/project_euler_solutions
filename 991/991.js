...
function primitiveSolutions(limit) {
    ...
    const alpha = 2.0 + Math.sqrt(3.0);
    const beta = (5.0 + Math.sqrt(21.0)) / 2.0;
    let k = 1n;
    while (true) {
...
function bruteForce(limit) {
    ...
    for (let a = 1n; a <= BigInt(limit); a++) {
        for (let b = 1n; b <= BigInt(limit) - a; b++) {
            const maxC = BigInt(limit) - a - b;
