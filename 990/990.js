...
const MAX_N = 50;
const MAX_TERMS = Math.floor((MAX_N + 1) / 2);
const MAX_CARRY = 25;
const CARRY_MIN = -MAX_CARRY;
const CARRY_MAX = MAX_CARRY;

function buildBinom(limit) {
    const comb = Array.from({ length: limit + 1 }, () => new Array(limit + 1).fill(0n));
...
function convolveSmall(poly, width) {
    const out = new Array(poly.length + width - 1).fill(0n);
...
function buildSumTables(limit) {
    const ways0to9 = new Array(limit + 1);
    ways0to9[0] = [1n];
    for (let p = 1; p <= limit; p++) {
        ways0to9[p] = convolveSmall(ways0to9[p - 1], 10);
    }
    const tables = Array.from({ length: limit + 1 }, () => new Array(limit + 1));
...
const BINOM = buildBinom(MAX_TERMS);
const SUM_TABLES = buildSumTables(2 * MAX_TERMS);
const transCache = new Map();

function getTransitions(activeLeft, activeRight, carry) {
...
function solve(limit) {
    const dp = Array.from({ length: limit + 1 }, () => new Map());
