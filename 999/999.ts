...
const MOD: bigint = 1_234_567_891n;
const INV_TWO: bigint = (MOD + 1n) / 2n;

const SMALL_W: bigint[] = [0n, 1n, 2n, -4n, -32n, -192n, 3584n, 77824n, 262144n];

function smallW(index: bigint): bigint {
    if (index < 0n) {
...
function edsBlock(n: bigint): bigint[] {
    if (n <= 4n) {
        const result: bigint[] = [];
        for (let i = 0n; i < 8n; i++) {
    ...
    function get(index: bigint): bigint {
        return source[Number(index - sourceStart)];
    }

    function odd(index: bigint): bigint {
    ...
    function even(index: bigint): bigint {
        const a = get(index);
