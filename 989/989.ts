...
const MOD: bigint = 1_000_000_009n;
const TARGET_LIMIT: bigint = 10n ** 14n;
const SMALL_NONPRIMITIVE_LIMIT: number = 8;
...
const SQRT5_MOD: bigint = tonelliShanks(5n, MOD);
const INV_SQRT5_MOD: bigint = modPow(SQRT5_MOD, MOD - 2n, MOD);
const INV2_MOD: bigint = (MOD + 1n) / 2n;
const PHI_MOD: bigint = (1n + SQRT5_MOD) * INV2_MOD % MOD;
const PHI_INV_MOD: bigint = modPow(PHI_MOD, MOD - 2n, MOD);
const PSI_MOD: bigint = (1n - SQRT5_MOD + MOD) % MOD;
const PHI_SQUARED_MOD: bigint = PHI_MOD * PHI_MOD % MOD;
const PHI_INV_SQUARED_MOD: bigint = PHI_INV_MOD * PHI_INV_MOD % MOD;

function buildSmallNonprimitiveterms(maxLimit: number): number[][] {
    const values: number[] = [];
...
const SMALL_NONPRIMITIVE_TERMS: number[][] = buildSmallNonprimitiveterms(SMALL_NONPRIMITIVE_LIMIT);

function evalSmallNonprimitivePair(limit: number, z1: bigint, z2: bigint): [bigint, bigint] {
...
function mobiusSieve(limit: number): Int8Array {
    ...
    for (let p = 2; p <= limit; p++) {
        ...
        for (let multiple = p; multiple <= limit; multiple += p) mu[multiple] = -mu[multiple] as (0 | 1 | -1);
        const square = p * p;
        if (square <= limit) {
...
function nonprimitivePair(limit: bigint, z1: bigint, z1Inv: bigint, z2: bigint, z2Inv: bigint): [bigint, bigint] {
...
function solve(limit: bigint): bigint {
    const root = Number(isqrt(limit));
    ...
    for (let g = 1; g <= root; g++) {
        ...
        const muG = mu[g];
        if (muG !== 0) {
            const scaledLimit = limit / gSquare;
            let psiPowG2: bigint, psiInvPowG2: bigint;
            if (g & 1) {
...
function bruteG(n: bigint): bigint {
    let count = 0n;
    for (let x = 0n; x < n; x++) {
...
function factorizeSmall(n: bigint): [bigint, number][] {
...
function gFromFactorization(n: bigint): bigint {
    if (n === 1n) return 1n;
...
function reducedPairCount(n: bigint): bigint {
    let count = 0n;
...
function bruteNonprimitivePair(limit: bigint, z1: bigint, z2: bigint): [bigint, bigint] {
    let total1 = 0n, total2 = 0n;
...
function bruteFibonacciSum(limit: bigint): bigint {
    const n = Number(limit);
    const fib: bigint[] = new Array(n + 1).fill(0n);
    if (n >= 1) fib[1] = 1n;
    if (n >= 2) fib[2] = 1n;
