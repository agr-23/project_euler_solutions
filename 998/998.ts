...
function pythagoreanPartners(limit: number): bigint[][] {
    const lim = BigInt(limit);
    const partners: bigint[][] = Array.from({ length: limit + 1 }, () => []);
...
function isMinimumSquare(sides: bigint[], twiceArea: bigint, squareSide: bigint): boolean {
    const ss = sides;
    const m = squareSide;
    const m2 = m * m;
