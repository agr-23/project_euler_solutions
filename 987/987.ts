...
const OVERLAP: boolean[][] = Array.from({ length: 10 }, () => new Array(10).fill(false));
for (let i = 0; i < 10; i++) {
...
const PERMS: bigint[][] = Array.from({ length: 5 }, () => new Array(5).fill(0n));
for (let n = 0; n < 5; n++) {
    PERMS[n][0] = 1n;
    let value = 1n;
    for (let k = 1; k < 5; k++) {
...
function coloringsOfAllSubsets(starts: number[]): bigint[] {
    const k = starts.length;
    const full = 1 << k;
    const adjacency: number[] = new Array(k).fill(0);
    for (let i = 0; i < k; i++) {
        for (let j = i + 1; j < k; j++) {
    ...
    const independent: boolean[] = new Array(full).fill(false);
    independent[0] = true;
    ...
    let dp: bigint[] = new Array(full).fill(0n);
    dp[0] = 1n;
    for (let c = 0; c < 4; c++) {
        const newDp: bigint[] = new Array(full).fill(0n);
...
function labeledCount(starts: number[]): bigint {
    const k = starts.length;
    const full = 1 << k;
    const colorings = coloringsOfAllSubsets(starts);
    const totalActive: number[] = new Array(13).fill(0);
    const activeMasksByRank: number[] = new Array(13).fill(0);
...
function* feasibleTypeCounts(target: number): Generator<number[]> {
    const counts: number[] = new Array(10).fill(0);
    const coverage: number[] = new Array(13).fill(0);
    function* backtrack(pos: number, remaining: number): Generator<number[]> {
        if (pos === 10) {
...
function countDisjointStraights(target: number): bigint {
    let total = 0n;
