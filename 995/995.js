...
const LIMIT = 20000;
const PRIME_SEARCH_LIMIT = 2000000;
...
const PRIMES = sieve(PRIME_SEARCH_LIMIT);
...
function divisorsFromFactorization(factors) {
    let divs = [1n];
...
function primitiveRoot(p, primeFactorsOfPMinus1) {
    if (p === 2n) return 1n;
    const m = p - 1n;
    for (let g = 2n; g < p; g++) {
        let ok = true;
...
function discreteLogTable(p, root) {
    const table = new Array(Number(p)).fill(-1n);
    let x = 1n;
...
function sForPrime(p) {
    ...
    const cItems = Array.from(leastPrimeForC.entries());

    const bestByM = new Map();
    ...
    const res = [dpValue.get(m), dpLog.get(m)];
    S_CACHE.set(p, res);
    return res;
}

function productT(limit) {
    let product = 1n;
    for (const p of PRIMES) {
        if (p >= limit) break;
...
function scientificFromInt(n, places = 5) {
    const digits = n.toString();
    const exponent = digits.length - 1;
    const significant = places + 1;
    let mantissaDigits, finalExp;
