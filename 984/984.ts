...
const TARGET_N: bigint = 10n ** 18n;

const EVEN_POLY: [bigint, bigint, number][] = [
    [31n, 40320n, 8],
    [31n, 3360n, 7],
    [67n, 1440n, 6],
    [41n, 320n, 5],
...
function computeEvenMod(n: bigint, mod: bigint): bigint {
    const powers: bigint[] = [1n];
    const x: bigint = ((n % mod) + mod) % mod;
    for (let i = 0; i < 8; i++) {
        powers.push((powers[powers.length - 1] * x) % mod);
    }
    let total: bigint = -419n;
...
function computeEvenInt(n: bigint): bigint {
    let numTotal: bigint = -419n;
    let denTotal: bigint = 1n;
    for (const [numerator, denominator, power] of EVEN_POLY) {
        const termNum: bigint = numerator * (n ** BigInt(power));
        const termDen: bigint = denominator;
        numTotal = numTotal * termDen + termNum * denTotal;
        denTotal = denTotal * termDen;
        const g: bigint = gcd(numTotal < 0n ? -numTotal : numTotal, denTotal < 0n ? -denTotal : denTotal);
        numTotal = numTotal / g;
        denTotal = denTotal / g;
    }
...
function runSolve(): bigint {
    const check1: bigint = computeEvenInt(100n);
    console.assert(check1 === 8658918531876n, `Expected 8658918531876 got ${check1}`);
    const check2: bigint = computeEvenMod(10000n, MODVAL);
