...
const LIMIT = 160;
const PREDICT_START_N = 33;
const SEARCH_WINDOW = 4096;

const EXCEPTION_H = new Map([
    [2, 3], [3, 5], [4, 7], [5, 11], [6, 13], [8, 21], [10, 31]
]);

function extinctForK1(n, k) {
    if (k === 0) return true;
    const size = n + 1;
    const last = size - 1;
...
function buildSSequence(maxN) {
    const s = new Array(maxN + 1).fill(0);
